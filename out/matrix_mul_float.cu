 ...
 */
 ...
 void file_read_matrix(float *h_a, int m, int n, const char *filename) {
     ...
     for (int i = 0; i < m; ++i) 
     {
         for (int j = 0; j < n; ++j) 
         {
             fscanf(fp, "%d", &h_a[i * n + j]);
         }
     }
     fclose(fp);
 }
 
 void print_matrix(float *h_a, int m, int n) {
     printf("matrix %d X %d:\n", m, n);
     for (int i = 0; i < m; ++i) 
     {
         for (int j = 0; j < n; ++j) 
 ...
 */
 int main(int argc, char const *argv[])
 {
     ...
     // read matrix A and B from input file
     
     file_read_matrix(h_a, m, n, filename);
     file_read_matrix(h_b, n, k, filename2);
